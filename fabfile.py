...
def run_install_scripts():
    ...
    locale_backup_name = 'locale-backup-{0}.sh'.format(project_name)
    ...
    with cd('$HOME/src/{0}/scripts'.format(project_name)):
        ...
        run('cp locale-backup.sh $HOME/bin/{0}'.format(locale_backup_name))
    ...
    with cd('$HOME/bin'):
        ...
        sed(locale_backup_name, 'INSERT_PROJECTNAME', project_name)
